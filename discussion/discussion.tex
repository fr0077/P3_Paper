...
\paragraph{スピンフリッパー間の飛行時間による効果}
2つのスピンフリッパーが距離$L_f$だけ離れているとする。時刻$t$に上流側のフリッパーを通過した速度$v$の中性子は時刻$t+L/v$に下流側のフリッパーを通過する。中性子がスピンフリッパー間を飛行するのにかかる時間$L_f/v$の間にスピンフリッパーの振動磁場$2B_r\cos\omega_s t$の位相は$\omega_s L_f/v$だけまわってしまう。この効果を取り入れるために下流側のスピンフリッパーの振動磁場を$2B_r\cos\omega_s (t+L_f/v)$としてみる。
\begin{figure}[h]
\centering
...
と表された入射中性子は領域VIIで
\begin{align}
\psi_\mathrm{VII}(x,t)=\begin{pmatrix} \cos \frac{\omega_r}{v}d & -i\sin\frac{\omega_r}{v}d \e^{-i\omega_s \left(t+\frac{L_f}{v}\right)} \\ -i\sin\frac{\omega_r}{v}d \e^{-i\omega_s \left(t+\frac{L_f}{v}\right)} &\cos \frac{\omega_r}{v}d\end{pmatrix} \begin{pmatrix} \e^{-i\frac{\omega}{v}d'} &0 \\0&\e^{i\frac{\omega}{v}d'} \end{pmatrix} \begin{pmatrix} \cos \frac{\omega_r}{v}d & -i\sin\frac{\omega_r}{v}d \e^{-i\omega_s t} \\ -i\sin\frac{\omega_r}{v}d \e^{-i\omega_s t} &\cos \frac{\omega_r}{v}d\end{pmatrix} \begin{pmatrix} 1\\0\end{pmatrix} \e^{i (k_0-\frac{\omega_z}{v})x}\e^{-i\omega_0t}
\end{align}
